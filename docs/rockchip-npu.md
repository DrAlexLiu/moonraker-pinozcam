...
> hardware requirements are identical -- same daemon, same models --
> so only the service user and the log location are rewritten.
...
The PiNozCam service user needs read/write access to the device that the
NPU driver exposes. It is whoever the systemd unit runs as -- normally the
same account that owns `printer_data`:

```bash
systemctl show -p User --value moonraker-pinozcam
```

Then test the NPU device reported above, replacing the example values:

```bash
service_user=$(systemctl show -p User --value moonraker-pinozcam)
npu_device=/dev/dri/card1
sudo -u "$service_user" test -r "$npu_device" \
    && sudo -u "$service_user" test -w "$npu_device" \
...
sudo usermod -aG "$npu_group" "$service_user"
```

Restart the service after changing group membership
(`sudo systemctl restart moonraker-pinozcam`). Do not add the
...
an `Inference backend ready` line in
`~/printer_data/logs/moonraker-pinozcam.log` is the authoritative
end-to-end check.
...
rules or add the service account to system groups. Those are deliberate
