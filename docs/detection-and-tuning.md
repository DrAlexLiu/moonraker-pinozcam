...
> Ported from the OctoPrint
> build's guide of the same name. The detection logic is not
> reimplemented here -- it is the same model, the same thresholds
> and the same sliding window -- so this text is upstream's, with
> only the places you change a setting rewritten for Klipper.
...
consistently detected, draw an **Undetect Zone** over it: open
`http://<printer>:58888` and press the eye-with-a-slash button.
...
|---|---|---:|---:|---:|
| Strictest | Fast GPU/NPU | 0.93 | 0.06 | 0.10 |
| Strict | Raspberry Pi 5, RK3588 | 0.90 | 0.05 | 0.08 |
| **Medium** (default) | Raspberry Pi 4, RK3566 | 0.87 | 0.04 | 0.05 |
| Loose | Raspberry Pi 3, Allwinner H616 | 0.84 | 0.03 | 0.03 |
| Loosest | Slow CPU / initial search | 0.80 | 0.02 | 0.01 |

These hardware labels are starting points, not compatibility
requirements. Editing one of the raw values changes the preset
label to Custom.

> The five presets are read from the OctoPrint build's own
> `static/js/pinozcam.js`, so both products offer the same steps.
> ⚠️ Upstream's copy of this guide still prints an older set of
> ratios (0.13/0.10/0.07) and older names (Lowest…Highest); the
> values above are what its code actually applies.

## Where these settings live

All of them are in `~/printer_data/config/moonraker-pinozcam.cfg`,
under `[detection]`, and Mainsail and Fluidd can edit that file in
the browser under **Machine → Configuration Files** -- no SSH.
The page at `http://<printer>:58888` edits the same values through
its settings dialog, including the five-step Sensitivity slider.

| in this guide | in the config file |
|---|---|
| Detection Score Threshold | `scores_threshold` |
| Failure Area Threshold | `img_sensitivity` |
| Failure Ratio Threshold | `failure_ratio` |
| Black Box Length | `count_time` |
| Action on Failure | `action` (0 alert, 1 pause, 2 stop) |
| Detection Start Delay | `ai_start_delay` |
