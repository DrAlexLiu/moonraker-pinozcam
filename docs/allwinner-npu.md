...
The PiNozCam service user needs read/write access to `/dev/vipcore`. Find
that user with:
...
sudo -u "$service_user" test -r /dev/vipcore \
    && sudo -u "$service_user" test -w /dev/vipcore \
...
⚠️ The service's own log is the place to look:
`~/printer_data/logs/moonraker-pinozcam.log`, which is also what Mainsail's
**Machine → Logs** page lists. `journalctl -u moonraker-pinozcam` carries
the same lines. `journalctl` for the
